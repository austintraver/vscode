...
suite('customizationMigration', () => {
	...
	test('splits candidates into focused, non-overlapping categories', () => {
		...
		const candidatesFor = (id: CustomizationMigrationCategoryId.PromptFiles | CustomizationMigrationCategoryId.UserData) => customizations
		...
		assert.deepStrictEqual({
			...
			sourceTypes: CUSTOMIZATION_MIGRATION_CATEGORIES.map(category => [
				category.id,
				category.migrationType === CustomizationMigrationType.McpServers ? [] : [...category.sourceTypes],
			]),
		}, {
			promptFiles: [
